...
function updateDashboard() {
  ...
  if (tileStudy) tileStudy.textContent = `${studyMin}<span class="tile-unit">min</span>`;
  ...
  if (todayLog) {
    ...
    if (tileSleep) tileSleep.textContent = `${todayLog.sleep_hours.toFixed(1)}<span class="tile-unit">hrs</span>`;

    const tileWater = document.getElementById("tile-water");
    if (tileWater) tileWater.textContent = `${todayLog.water_glasses}<span class="tile-unit">glasses</span>`;
